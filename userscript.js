// ==UserScript==
// @name         What.cd Youtube discover
// @version      1.2.0
// @description  Adds youtube videos to album playlists and youtube search links to the artist page.
// @include      *what.cd/torrents.php*
// @include      *what.cd/artist.php*
// @updateURL    https://raw.githubusercontent.com/Lemmata/what.cd-track-previews/master/userscript.js
// @grant        GM_getValue
// @grant        GM_setValue
// @require      http://code.jquery.com/jquery-latest.js
// ==/UserScript==

//add in youtube link on artistpage
var artistMatcher = /.*what\.cd\/artist.php\?id=.*/i;
if (artistMatcher.test(window.location.href)){
    
function youtubelink(artistName){
	return "</img><a href='"+
        'https://www.youtube.com/results?search_query=' + encodeURIComponent(artistName)
        +"'>Search Youtube</a>";
}

/* Store an array of places to link to */
var linklist = [youtubelink];

var artistName = $("div[class='header'] > h2").text();
                
$(".box_image").after(function(){
    var extra = "<div class='box'> <div class=head> Links </div> <ul class='stats nobullet'>";
    for (var i = 0; i < linklist.length; i++){
    	extra = extra + "<li>" + linklist[i](artistName) + "</li>";
    }
    return extra + "</ul> </div>";
});
}else{  //show for albums
